...
<section class="relative py-12 md:py-20 bg-gradient-to-br from-slate-50 to-blue-50 overflow-hidden">
    ...
    <div class="absolute inset-0">
        ...
        <svg class="absolute top-0 w-full h-16 md:h-24 opacity-30" viewBox="0 0 1200 120" preserveAspectRatio="none">
            <path d="M0,60 C300,20 600,100 900,60 C1050,40 1150,80 1200,60 L1200,0 L0,0 Z" fill="url(#waveGradient)"></path>
            <defs>
    ...
    <div class="relative z-10 container mx-auto px-4 md:px-6">
        ...
        <div class="text-center mb-12 md:mb-16">
            <div class="inline-block mb-4">
                <span class="px-3 py-1 md:px-4 md:py-2 bg-sky-100 text-sky-600 rounded-full text-xs md:text-sm font-semibold tracking-wide uppercase">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 mb-8 md:mb-12">
            ...
            <div class="group relative overflow-hidden rounded-2xl md:rounded-3xl destination-card cursor-pointer">
                <div class="aspect-[4/5] bg-gradient-to-br {{ $destination['gradient'] }} destination-image relative" style="background-image: url('{{ $destination['image'] }}');">
                    <!-- Price Tag -->
                    <div class="absolute top-3 right-3 md:top-4 md:right-4 px-2 py-1 md:px-3 md:py-1 bg-white/90 backdrop-blur-sm rounded-full text-sky-600 font-bold text-xs md:text-sm price-tag">
                        {{ $destination['price'] }}
                    </div>
                    
                    <!-- Heart Icon -->
                    <button class="absolute top-3 left-3 md:top-4 md:left-4 w-8 h-8 md:w-10 md:h-10 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center text-white hover:bg-white/30 transition-all duration-300 heart-btn">
                        ...
                        <div class="rating-stars flex space-x-1">
                            @for($i = 1; $i <= 5; $i++)
                                @if($i <= floor($destination['rating']))
                                    <svg class="w-3 h-3 md:w-4 md:h-4 fill-current text-yellow-400" viewBox="0 0 20 20">
                                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                    </svg>
                                @else
                                    <svg class="w-3 h-3 md:w-4 md:h-4 fill-current text-white/30" viewBox="0 0 20 20">
                                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                    </svg>
                                @endif
                            @endfor
                        </div>
                        <span class="ml-2 text-xs md:text-sm text-white/90">{{ $destination['rating'] }} ({{ $destination['reviews'] }} reviews)</span>
                    </div>
                    <h3 class="text-lg md:text-xl lg:text-2xl font-bold mb-2">{{ $destination['name'] }}</h3>
                    <p class="text-white/80 text-xs md:text-sm mb-3 line-clamp-2">{{ $destination['description'] }}</p>
                    <div class="flex items-center justify-between">
                        <span class="text-xs md:text-sm text-white/70">{{ $destination['duration'] }}</span>
                        <button class="px-3 py-1 md:px-4 md:py-2 bg-sky-500 hover:bg-sky-400 rounded-full text-xs md:text-sm font-semibold transition-all duration-300 transform hover:scale-105">
                            Book Now
                        </button>
                    </div>
                </div>

    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8 pb-20 relative z-20">
                ...
                <div class="rounded-2xl shadow-lg p-8 mb-8">
                    ...
                    <form action="{{ route('bookings.index') }}" method="GET" class="max-w-md mx-auto">
                        <div class="mb-4">
                            ...
                            <input 
                                ...
                                value="{{ old('email', session('guest_booking_email')) }}"
                            >
                            @error('email')
                    ...
                    <div class="mt-6 text-center">
                        <p class="text-sm text-gray-600">
                            ...
                            <a href="{{ route('register') }}" class="text-teal-600 hover:text-teal-700 font-semibold hover:underline">
                                Create one here
                            </a> 
        ...
        @if($bookings->count() > 0)
            <div class="mb-6 flex items-center justify-between">
                ...
                @auth
                    <a href="{{ route('make-a-request') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition text-sm font-medium">
            ...
            <div class="space-y-6">
                @foreach($bookings as $booking)
                    <div class=" rounded-2xl shadow-md overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-xl border border-gray-100">
                        <!-- Header -->
                        <div class="bg-gradient-to-r from-teal-50 to-cyan-50 px-6 py-4 border-b border-gray-100">
                            <div class="flex flex-wrap items-center justify-between gap-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 bg-gradient-to-br from-teal-700/10 to-teal-500/15">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-600">
                                            Booking #{{ $booking->id }} • {{ $booking->created_at->format('M d, Y') }}
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div class="flex items-center gap-3">
                                    @if($booking->user_id)
                                        <span class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-semibold bg-green-100 text-green-700">
                                    ...
                                    @else
                                        <span class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-semibold bg-gray-100 text-gray-600">
                                            <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <div class="p-6">
                            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                                <!-- Travel Dates -->
                                <div>
                                    <h4 class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Travel Dates</h4>
                                    <div class="space-y-2">
                                        <div class="flex items-center gap-2 text-sm">
                                            ...
                                            <span class="text-gray-900">{{ $booking->departure_date ? $booking->departure_date->format('M d, Y') : 'N/A' }}</span>
                                        </div>
                                        @if($booking->return_date)
                                            <div class="flex items-center gap-2 text-sm">
                                                ...
                                                <span class="text-gray-900">{{ $booking->return_date->format('M d, Y') }}</span>
                                            </div>
                                            <div class="mt-2 inline-flex items-center gap-1 px-3 py-1 bg-teal-50 text-teal-700 rounded-full text-xs font-medium">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                                </svg>
                                                {{ $booking->trip_duration }} {{ Str::plural('night', $booking->trip_duration) }}
                                            </div>
                                        @endif
                                ...
                                <!-- Travelers -->
                                <div>
                                    <h4 class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Travelers</h4>
                                    <div class="space-y-2 text-sm">
                                        ...
                                        @endif
                                        <div class="mt-2 inline-flex items-center gap-1 px-3 py-1 bg-gray-100 text-gray-700 rounded-full text-xs font-medium">
                                            Total: {{ $booking->total_travelers }} {{ Str::plural('traveler', $booking->total_travelers) }}
                                        </div>
                                    </div>
                                </div>
                                ...
                                <!-- Contact Information -->
                                <div>
                                    <h4 class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Contact Details</h4>
                                    <div class="space-y-2 text-sm">
                            ...
                            <div class="mt-6 pt-6 border-t border-gray-100">
                                <div class="grid md:grid-cols-2 gap-6">
                                    ...
                                    <div class="space-y-3">
                                        @if($booking->trip_type)
                                            <div class="flex items-center gap-2 text-sm">
                                                ...
                                                <span class="text-gray-900">{{ ucwords(str_replace('_', ' ', $booking->trip_type)) }}</span>
                                            </div>
                                        @endif
                                        ...
                                        @if($booking->budget)
                                            <div class="flex items-center gap-2 text-sm">
                                                ...
                                                <span class="text-gray-900">{{ ucwords(str_replace('_', ' ', $booking->budget)) }}</span>
                                            </div>
                                        @endif
                                        
                                        @if($booking->accommodation)
                                            <div class="flex items-center gap-2 text-sm">
                                                <span class="text-gray-600 font-medium min-w-[120px]">Accommodation:</span>
                                                <span class="text-gray-900">{{ ucwords(str_replace('_', ' ', $booking->accommodation)) }}</span>
                                            </div>
                                        @endif
                                    </div>
                                    ...
                                    <div class="space-y-3">
                                        @if($booking->nationality)
                                            <div class="flex items-center gap-2 text-sm">
                                                <span class="text-gray-600 font-medium min-w-[120px]">Nationality:</span>
                                ...
                                @if(!empty($booking->services_list) && count($booking->services_list) > 0)
                                    <div class="mt-4">
                                        <h5 class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Additional Services</h5>
                                        <div class="flex flex-wrap gap-2">
                                            @foreach($booking->services_list as $service)
                                                <span class="inline-block px-3 py-1 bg-teal-50 text-teal-700 rounded-md text-xs font-medium border border-teal-200">
                                ...
                                @if($booking->special_requests)
                                    <div class="mt-4 p-4 bg-gray-50 rounded-lg">
                                        <h5 class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Special Requests</h5>
                                        <p class="text-sm text-gray-700 leading-relaxed">{{ $booking->special_requests }}</p>
                                    </div>
                                @endif
                            </div>

...
<div class="min-h-screen font-['Lato',sans-serif] bg-gray-50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8 pb-20 relative z-20">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <!-- Total Bookings -->
            <div class="bg-white rounded-2xl shadow-lg p-6 border-l-4 border-teal-600 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
                <div class="flex items-center justify-between">
                    <div>
                ...
                <div class="mt-4 flex items-center text-sm">
                    <a href="{{ route('user.bookings.contacts') }}" class="text-teal-600 hover:text-teal-700 font-medium hover:underline">
                        View all bookings →
                    </a>
                </div>
            </div>
            ...
            <!-- Upcoming Trips -->
            <div class="bg-white rounded-2xl shadow-lg p-6 border-l-4 border-blue-600 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
                <div class="flex items-center justify-between">
                    <div>
            ...
            <!-- Total Inquiries -->
            <div class="bg-white rounded-2xl shadow-lg p-6 border-l-4 border-purple-600 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
                <div class="flex items-center justify-between">
                    <div>
                ...
                <div class="mt-4 flex items-center text-sm">
                    <a href="{{ route('inquiries.index') }}" class="text-purple-600 hover:text-purple-700 font-medium hover:underline">
                        View all inquiries →
                    </a>
                </div>
            </div>
            ...
            <!-- Total Travelers -->
            <div class="bg-white rounded-2xl shadow-lg p-6 border-l-4 border-orange-600 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
                <div class="flex items-center justify-between">
                    <div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6">
                <div class="flex items-center justify-between mb-6">
                    ...
                    <a href="{{ route('user.bookings.contacts') }}" class="text-sm text-teal-600 hover:text-teal-700 font-medium hover:underline">
                        View All
                    </a>
                </div>
                        ...
                        @foreach($recentBookings as $booking)
                            <div class="flex items-start gap-4 p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-semibold text-gray-900 truncate">{{ $booking->destination }}</p>
                                    <p class="text-xs text-gray-500 mt-1">
                                        {{ $booking->departure_date ? $booking->departure_date->format('M d, Y') : 'Date pending' }}
                                        @if($booking->total_travelers)
                                            • {{ $booking->total_travelers }} {{ Str::plural('traveler', $booking->total_travelers) }}
                                        @endif
                                    </p>
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        <a href="{{ route('book-travel') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-teal-600 text-white text-sm rounded-lg hover:bg-teal-700 transition">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6">
                <div class="flex items-center justify-between mb-6">
                    ...
                    <a href="{{ route('inquiries.index') }}" class="text-sm text-purple-600 hover:text-purple-700 font-medium hover:underline">
                        View All
                    </a>
                </div>
                        ...
                        @foreach($recentInquiries as $inquiry)
                            <div class="flex items-start gap-4 p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-xs text-gray-500 mt-1 line-clamp-2">{{ Str::limit($inquiry->message, 80) }}</p>
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        <a href="{{ route('contact') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-purple-600 text-white text-sm rounded-lg hover:bg-purple-700 transition">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                            </svg>
                            Submit Inquiry
...
<script>
    ...
    if (tripTypesCtx) {
        ...
        new Chart(tripTypesCtx, {
            type: 'pie',
            data: {
                labels: Object.keys(tripTypeData).map(type => type.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())),
                datasets: [{
    ...
    if (budgetCtx) {
        ...
        new Chart(budgetCtx, {
            type: 'bar',
            data: {
                labels: Object.keys(budgetData).map(budget => budget.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())),
                datasets: [{
                    label: 'Bookings',
